...
class EcopulseAppSupabaseUser extends BaseAuthUser {
  EcopulseAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> ecopulseAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = EcopulseAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
